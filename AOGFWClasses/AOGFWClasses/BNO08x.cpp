...
void BNO080::initialize() {
	...
	for (int16_t i = 0; i < nrBNO08xAdresses; i++)
	{
		...
		//logger.LogMessage("\r\nChecking for BNO08X on " + String(bno08xAddress, HEX),Logger::IMU);
		...
		if (error == 0)
		{
			...
			if (begin(bno08xAddress, ImuWire))
			{
				...
				enableGameRotationVector(REPORT_INTERVAL);
				devicePresent = true;
				return;
			}
			else
...
IMU::IMUData BNO080::getIMUData(bool invertRoll, bool useYAxis) {
	if (dataAvailable() == true)
	{
		...
		//get quaternion
		getQuat(dqx, dqy, dqz, dqw, dacr, dac);
		...
		imuData.yaw = atan2(t3, t4);

		// Convert yaw to degrees x10
		imuData.correctionHeading = -imuData.yaw;
		imuData.yaw = (int16_t)((imuData.yaw * -RAD_TO_DEG_X_10));
		if (imuData.yaw < 0) imuData.yaw += 3600;
		...
		if (useYAxis)
		{
			imuData.roll = asin(t2) * RAD_TO_DEG_X_10;
			imuData.pitch = atan2(t0, t1) * RAD_TO_DEG_X_10;
		}
		else
		{
			imuData.pitch = asin(t2) * RAD_TO_DEG_X_10;
			imuData.roll = atan2(t0, t1) * RAD_TO_DEG_X_10;
		}
